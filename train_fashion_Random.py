...
from netsn.loss import YOLOLoss
from netsn.network import MyNetBody
...
torch.set_num_threads(cpu_num-1)
...
def fit_ont_epoch(net,yolo_losses,epoch,epoch_size,epoch_size_val,gen,genval,Epoch,cuda):
    ...
    print('Total Loss: %.4f loc: %.4f conf: %.4f cls: %.4f img: %.4f\n Val Loss: %.4f loc: %.4f conf: %.4f cls: %.4f img: %.4f' % (total_loss/(epoch_size),loss_loc/(epoch_size),loss_conf/(epoch_size),loss_cls/(epoch_size),loss_img/(epoch_size),
    val_loss/(epoch_size_val),val_loc/(epoch_size_val),val_conf/(epoch_size_val),val_cls/(epoch_size_val),val_img/(epoch_size_val)))
    ...
    with open('%s/%s/output.txt'%(logs_path, Origin_Time), 'a+') as f:
        ...
        f.write('Total Loss: %.4f loc: %.4f conf: %.4f cls: %.4f img: %.4f\n Val Loss: %.4f loc: %.4f conf: %.4f cls: %.4f img: %.4f\n' % (total_loss/(epoch_size),loss_loc/(epoch_size),loss_conf/(epoch_size),loss_cls/(epoch_size),loss_img/(epoch_size),
    val_loss/(epoch_size_val),val_loc/(epoch_size_val),val_conf/(epoch_size_val),val_cls/(epoch_size_val),val_img/(epoch_size_val)))
...
if __name__ == "__main__":
    ...
    #-------------------------------------------#
    Use_model = False
    model_path = "logs/Sat_Jul_24_01:34:44_2021/Epoch12-Total_Loss33.9051-Val_Loss37.2384.pth"
    ...
    model = MyNetBody(len(anchors[0]),num_classes, M)
    if Cuda:
    ...
    gen = GenerateInput(Batch_size, lines[:num_train], indexs[:num_train], M, image_size_lab, dataset='fashion_rd').generate()
    gen_val = GenerateInput(Batch_size, lines[num_train:], indexs[num_train:], M, image_size_lab, dataset='fashion_rd').generate()
    ...
    for lr,epoch,inlayer in train_mode:
        ...
        for param in model.forhead.inlayer.parameters():

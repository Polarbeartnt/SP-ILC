...
def Precision_Recall(ppath, fpath, iou=0.5, confbar=[float(x)/10 for x in range(10)]):
    conf_p = ppath.replace('.txt', '_conf.txt')
    with open(ppath, 'r') as f:
        predict_boxes = np.array(f.read().split(), dtype=np.int32)
        predict_boxes = predict_boxes.reshape([predict_boxes.shape[0]//5, 5])
    with open(conf_p, 'r') as f:
        confs = np.array(f.read().split(), dtype=np.float32)
    with open(fpath, 'r') as f:
        gr = np.array(f.read().split(), dtype=np.int32)
        gr = gr.reshape([gr.shape[0]//5, 5])
    pr = []
    for conf in confbar:
        predict_box = predict_boxes[confs>=conf]
        find = 0
        correct = 0
        flag = [False, False]
        for object in gr:
            for predict in predict_box:
                if numpyIOU(object, predict) >= iou:
                    flag[0] = True
                    if object[4] == predict[4]:
                        flag[1] = True
            if flag[0]:
                find += 1
                flag[0] = False
            if flag[1]:
                correct += 1
                flag[1] = False
        pr.append([find, len(gr), correct, len(predict_box), conf])
    return pr

def numpyIOU(box1, box2):
    b1_x1, b1_y1, b1_x2, b1_y2 = box1[0], box1[1], box1[2], box1[3]
    b2_x1, b2_y1, b2_x2, b2_y2 = box2[0], box2[1], box2[2], box2[3]
    ...
    inter_area = np.clip(inter_rect_x2 - inter_rect_x1 + 1, 0, 1e10) * \
                 np.clip(inter_rect_y2 - inter_rect_y1 + 1, 0, 1e10)

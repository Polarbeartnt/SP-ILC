...
from netslarge.network import MyNetBody as LargeNet
from netsTA.network import MyNetBody as TrainANet
import csv
...
predictor = Predictor(_defaults = {
            "model_path"        : 'logs/随机矩阵/Epoch28-Total_Loss22.7959-Val_Loss27.5650.pth', #path of your weights-file
            ...
            "confidence"        : 0.04,# confidence threshold can be modified here 
            "iou"               : 0.5,
            "cuda"              : False,
            "MyNetBody"         : SimpleNet, # which net used
            "FineTune"          : None,
            "FT_log"            : '',
            "dataset"           : ''
        })

Path = ['Single_Object', 'Multiple_Objects']
for path in Path:
    mkdir('test_results', path)
    f = open('test_results/results_%s.txt'%(path), 'a+')
    f.write(predictor.model_path+'\n')
    with open('expdata_ft/test_ref_%s.txt'%(path), 'r') as g:
    ...
    for p in exfile:
        name = p.split('/')[-1]
        fig = np.array(Image.open(p.replace('Objects_ref', 'Objects_fig').replace('Object_ref/', 'Object_fig/').replace('.txt', '.png')).convert('L'))
        rec = np.array(Image.open('test_results/' + path + '_predict/'+name.replace('.txt', '.png')).convert('L'))
        pr = Precision_Recall('test_results/'+path+'_bbox/'+name, p.replace('Objects_ref', 'Objects_label').replace('Object_ref/', 'Object_label/'),confbar=[x/20 + 0.05 for x in range(20)])
...
for path in Path:
    ...
    for p in exfile:
        ...
        rec = np.array(Image.open('test_results/' + path + '_predict/'+name.replace('.txt', '.png')).convert('L'))
        pr = Precision_Recall('test_results/'+path+'_bbox/'+name, p.replace('test', 'test_label'),confbar=[x/20 + 0.05 for x in range(20)])

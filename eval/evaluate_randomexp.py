...
for path in Path:
    ...
    for p in exfile:
        ...
        pr = Precision_Recall('test_results/'+path+'_bbox/'+name, p.replace('Objects', 'Objects_label').replace('Object/', 'Object_label/'))
    ...
    with open('test_results/pr_%s.csv'%(path), 'a+') as f:
        f_csv = csv.writer(f)

...
predictor = Predictor(_defaults = {
            ...
            "confidence"        : 0.9,# confidence threshold can be modified here 
            "iou"               : 0.5,
            "cuda"              : False,
...
prf = open('test_results/pr.csv', 'w')
for path in Path:
    ...
    for p in exfile:
        ...
        pr = Precision_Recall('test_results/'+path+'_bbox/'+name, p.replace('test', 'test_label'), confbar=[0.9])
